...
func newClassPrivateFieldsTransformer(emitContext *printer.EmitContext) *transformers.Transformer {
...
func (tx *classPrivateFieldsTransformer) transformConstructor(constructor *ast.ConstructorDeclaration, container *ast.ClassLikeDeclaration) *ast.ConstructorDeclaration {
	...
	if tx.EmitContext().GetClassFacts()&ClassFactsWillHoistInitializersToConstructor == 0 {
		return constructor
	}
...
// func (tx *ClassPrivateFieldsTransformer) addInstanceMethodStatements(statements []*ast.Statement, methods []*ast.Node, receiver *ast.LeftHandSideExpression) []*ast.Statement {
// 	if len(methods) == 0 {
// 		return statements
// 	}
